...
- **코드 하이라이팅**: Pygments로 키워드/문자열/주석 색상 구분 (펜스 언어 인식)
...
- **Web UI**: 모바일 친화 디자인, 드래그&드롭 업로드, 옵션 토글
...
| `--font PATH` | 모든 역할(regular/bold/code)에 단일 .ttf 강제 적용 |
...
- **커스텀 CSS** 파일 업로드 (선택)
- **PDF로 변환** 버튼 클릭 → 변환된 PDF가 다운로드로 스트리밍
